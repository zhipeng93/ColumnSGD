...
object baseApp{
  def main(args: Array[String]): Unit ={
    ...
    val sparkConf = new SparkConf().setAppName("baseline-" + model_name)
    val sparkContext = new SparkContext(sparkConf)

    // RDD[(Int, Array[IndexedDataPoint])], Array[Double])
    val data_rdd: RDD[LabeledPoint] = MLUtils.loadLibSVMFile(sparkContext, in_path, num_features, minPartitions = num_partitions).cache()
